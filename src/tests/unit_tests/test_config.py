...
def test_development_config(test_app):
    ...
    assert test_app.config['SECRET_KEY'] == 'very_secret'
    assert not test_app.config['TESTING']
    assert test_app.config['SQLALCHEMY_DATABASE_URI'] == os.environ.get('PG_URL')
...
def test_testing_config(test_app):
    ...
    assert test_app.config['SQLALCHEMY_DATABASE_URI'] == os.environ.get('PG_TEST_URL')
